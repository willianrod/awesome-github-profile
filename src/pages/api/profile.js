...
class SVGProfile {
   ...
   */
  _createContainer() {
    return this.canvas.rect(WIDTH, HEIGHT).fill(this.theme.background);
  }
  ...
  _renderWelcomeText() {
    ...
    group.text(this.job).fill(this.theme.textColor).opacity(0.6).move(0, 59).font({
      size: 13,
      weight: '300'
    });
  ...
  async _renderRecentActivity() {
    ...
    languagesToShow.forEach((language) => {
      ...
      group.rect(256, 23).radius(4).fill(this.theme.cardColor).opacity(0.06);
      group
        .rect(50, 19)
        .radius(4)
      ...
      group
        ...
        .fill('#fff')
        .font({ size: 12 })
        .move(cardWidth - timeCardWidth + 1, 0);
  ...
  _renderGithubStats({ repositories, pullRequests, contributionsCollection }) {
    const totalStars = repositories.nodes.reduce(
      (acc, curr) => (acc += curr.stargazers.totalCount),
      0
    );
    const commits =
    ...
    this._createGithubStatsCard({ label: 'Repos', value: repositories.totalCount }).move(
      initialCardLeftOffset,
      590
    );
    this._createGithubStatsCard({ label: 'Stars', value: totalStars }).move(
      initialCardLeftOffset,
      647
    );
    this._createGithubStatsCard({ label: 'Commits', value: commits }).move(
      initialCardLeftOffset - STATS_CARD_WIDTH - 16,
      590
    );
    this._createGithubStatsCard({ label: "PR's", value: pullRequests.totalCount }).move(
      initialCardLeftOffset - STATS_CARD_WIDTH - 16,
      647
    );
  }
